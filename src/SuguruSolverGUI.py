...
class SuguruSolverGUI:
        ...
        def __init__(self, master):
                ...
                self.bwidth = 15 # button width
                self.bheight = 1 # button height
                self.fborder = 1 # frame border thickness
                self.frelief = 'solid' # frame border relief
                self.fpadx = 15 # frame inner horizontal padding
                self.fpady = 15 # frame inner vertical padding

                # define top level frames
                self.left_buttons_frame = tk.Frame(master, width=200)
                self.left_buttons_frame.grid(row=0, column=0)
                self.middle_grid_frame = tk.Frame(master, width=200)
                self.middle_grid_frame.grid(row=0, column=1)
                self.right_buttons_frame = tk.Frame(master, width=200)
                self.right_buttons_frame.grid(row=0, column=2)
                self.bottom_text_frame = tk.Frame(master, width=200)
                self.bottom_text_frame.grid(row=1, column=0, columnspan=3)
                
                # layout of middle frame
                
                # define label
                label = tk.Label(self.middle_grid_frame, text='Suguru view')
                label.grid(row=0, column=0)

                # set common setting for grid and layout frame
                self.glargs = {'row':1, 'column':0, 'padx':10, 'pady':10}

                # define a frame for the suguru layout cells          
                self.layout_frame = tk.Frame(self.middle_grid_frame, width=200)
                self.layout_frame.grid(**self.glargs)

                # define a frame for the suguru grid cells          
                self.grid_frame = tk.Frame(self.middle_grid_frame, width=200)
                #self.grid_frame.grid(**self.glargs)

                # define toggle button between layout and grid editing
                self.editmode = tk.StringVar()
                self.editmode.set("L")
                self.edit_toggle_frame = tk.Frame(self.middle_grid_frame, width=200)
                self.edit_toggle_frame.grid(row=2, column=0)
                self.edit_layout_button = tk.Radiobutton(self.edit_toggle_frame,
                        text='Layout grid', indicatoron=False,
                        variable=self.editmode, value="L", command=self.seteditmode,
                        width=self.bwidth, height=self.bheight)
                self.edit_layout_button.grid(row=0, column=0)
                self.edit_grid_button = tk.Radiobutton(self.edit_toggle_frame,
                        text='Digit grid', indicatoron=False,
                        variable=self.editmode, value="G", command=self.seteditmode,
                        width=self.bwidth, height=self.bheight)
                self.edit_grid_button.grid(row=0, column=1)

                # define a frame for the candidate cells
                self.candidate_frame = tk.Frame(self.middle_grid_frame, height=30, width=200)
                #self.candidate_frame.grid(row=4, column=0, padx=10, pady=10)

                # initialize the grid with default size
                self.initgrid()

                # layout of right frame

                # add a label
                label = tk.Label(self.right_buttons_frame, text='Editing and solving')
                label.grid(row=0, column=0)

                # define a frame for the layout buttons
                self.layout_buttons_frame = tk.Frame(self.right_buttons_frame, width=200,
                        borderwidth=self.fborder, relief=self.frelief,
                        padx=self.fpadx, pady=self.fpady)
                self.layout_buttons_frame.grid(row=1, column=0)
                # add a label
                label = tk.Label(self.layout_buttons_frame, text='Layout')
                label.grid(row=0, column=0)
                ...
                self.change_dim_button = tk.Button(self.layout_buttons_frame,
                        ...
                        width=self.bwidth, height=self.bheight)
                self.change_dim_button.grid(row=1, column=0)
                # add a button to read the currently entered layout
                self.update_layout_button = tk.Button(self.layout_buttons_frame,
                        text='Update',
                        command=self.update_layout_to_grid,
                        width=self.bwidth, height=self.bheight)
                self.update_layout_button.grid(row=2, column=0)
                # add a button to clear the layout
                self.clear_layout_button = tk.Button(self.layout_buttons_frame,
                        text='Clear',
                        command=self.clear_layout,
                        width=self.bwidth, height=self.bheight)
                self.clear_layout_button.grid(row=3, column=0)

                # define a frame for the solve buttons                          
                self.solve_buttons_frame = tk.Frame(self.right_buttons_frame, width=200,
                        borderwidth=self.fborder, relief=self.frelief,
                        padx=self.fpadx, pady=self.fpady)
                self.solve_buttons_frame.grid(row=2, column=0)
                # add a label
                label = tk.Label(self.solve_buttons_frame, text='Solving')
                label.grid(row=0, column=0)
                # define mode button 
                ...
                self.solve_button = tk.Button(self.solve_buttons_frame, text='Solve',command=self.solve,
                        width=self.bwidth, height=self.bheight)
                self.solve_button.grid(row=1, column=0)
                self.abort_button = tk.Button(self.solve_buttons_frame, text='Abort', command=self.abort,
                        width=self.bwidth, height=self.bheight)
                self.abort_button.grid(row=2, column=0)
                self.hint_button = tk.Button(self.solve_buttons_frame, text='Hint', command=self.hint,
                        width=self.bwidth, height=self.bheight)
                self.reduce_button = tk.Button(self.solve_buttons_frame, text='Reduce', command=self.reduce,
                        width=self.bwidth, height=self.bheight)
                ...
                # add a label
                label = tk.Label(self.left_buttons_frame, text='I/O actions')
                label.grid(row=0, column=0)

                # define a frame for saving and loading buttons
                self.io_buttons_frame = tk.Frame(self.left_buttons_frame, width=200,
                        borderwidth=self.fborder, relief=self.frelief,
                        padx=self.fpadx, pady=self.fpady)
                self.io_buttons_frame.grid(row=1, column=0)
                # add a label
                label = tk.Label(self.io_buttons_frame, text='Load/Save')
                label.grid(row=0, column=0)
                # button for loading
                self.load_button = tk.Button(self.io_buttons_frame,text='Load',command=self.load,
                        width=self.bwidth, height=self.bheight)
                self.load_button.grid(row=1,column=0)
                # button for saving
                self.save_button = tk.Button(self.io_buttons_frame, text='Save', command=self.save,
                        width=self.bwidth, height=self.bheight)
                self.save_button.grid(row=2, column=0)
                # add a button to upload an image
                self.load_image_button = tk.Button(self.io_buttons_frame,
                        text='Load from image',
                        command=self.load_image,
                        width=self.bwidth, height=self.bheight)
                self.load_image_button.grid(row=3, column=0)
                
                # define a frame for clearing and closing buttons
                self.close_buttons_frame = tk.Frame(self.left_buttons_frame, width=200,
                        borderwidth=self.fborder, relief=self.frelief,
                        padx=self.fpadx, pady=self.fpady)
                self.close_buttons_frame.grid(row=2, column=0)
                # add a label
                label = tk.Label(self.close_buttons_frame, text='Clear/Close')
                label.grid(row=0, column=0)
                # clear button
                self.clear_button = tk.Button(self.close_buttons_frame, text='Clear', command=self.clear,
                        width=self.bwidth, height=self.bheight)
                self.clear_button.grid(row=1, column=0)
                # close button
                self.close_button = tk.Button(self.close_buttons_frame, text='Close', command=master.destroy,
                        width=self.bwidth, height=self.bheight)
                self.close_button.grid(row=2, column=0, columnspan=2)

                # bottom frame layout

                self.messages_text = scrtxt.ScrolledText(self.bottom_text_frame, width=85, height=15)
                self.messages_text.grid(row=0, column=0)
                initstring = 'Welcome to the Suguru Solver!\n'
                initstring += 'Quick guide:\n'
                initstring += '- Set the correct number of rows and columns in "Change size".\n'
                initstring += '- Define the layout by entering numbers in the layout grid\n'
                initstring += '  (equal numbers for all cells in a block, starting from zero).\n'
                initstring += '- Press "Update". Check that the layout (in colors) is correct.\n'
                initstring += '- Switch to "Digit grid".\n'
                initstring += '- Fill the known cells with the correct values.\n'
        ...
        def seteditmode(self):
                ### toggle between layout and grid editing
                if self.editmode.get()=='L':
                    self.grid_frame.grid_forget()
                    self.layout_frame.grid(**self.glargs)
                if self.editmode.get()=='G':
